...
def load_scope_patterns(checks_dir: str = "scripts/checks", teams: list[str] | None = None) -> dict:
    ...
    Raises:
        ...
        ValueError: if a config file contains invalid JSON
    """
    checks_path = Path(checks_dir)
    core = _load_json(checks_path / "core" / "scope_patterns.json")

    allowed_test_levels = list(core.get("allowed_test_levels", []))
    forbidden_test_levels = list(core.get("forbidden_test_levels", []))
    forbidden_patterns = list(core.get("forbidden_patterns", []))

    for team_config in _iter_team_configs(checks_path, teams, "scope_patterns.json"):
        allowed_test_levels.extend(team_config.get("allowed_test_levels", []))
        forbidden_test_levels.extend(team_config.get("forbidden_test_levels", []))
        forbidden_patterns.extend(team_config.get("forbidden_patterns", []))
...
def load_boilerplate_patterns(checks_dir: str = "scripts/checks", teams: list[str] | None = None) -> dict:
    ...
    patterns = {category: list(core_patterns.get(category, [])) for category in _BOILERPLATE_CATEGORIES}

    for team_config in _iter_team_configs(checks_path, teams, "boilerplate_patterns.json"):
        team_patterns = team_config.get("patterns", {})
        for category in _BOILERPLATE_CATEGORIES:
            patterns[category].extend(team_patterns.get(category, []))
